...
Questo è un esempio di come si configura un middleware plugin. All middleware plugins must be defined in the **middlewares** namespace.
...
Questo è un esempio di come si configura un plugin di archiviazione. All storage plugins must be defined in the **store** namespace.
...
Verdaccio allows to replace the User Interface with a custom one, we call it **theme**. Di default, utilizza `@verdaccio/ui-theme` che è integrato, tuttavia è possibile usare qualcosa di diverso installando il proprio plugin.
