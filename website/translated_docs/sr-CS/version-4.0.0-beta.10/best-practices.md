...
If some package doesn't exist in the storage, server will try to fetch it from npmjs.org. U slučaju da npmjs.org nije u funkciji, preuzeće se iz cache-a. **Verdaccio will download only what's needed (= requested by clients)**, and this information will be cached, so if client will ask the same thing second time, it can be served without asking npmjs.org for it.
