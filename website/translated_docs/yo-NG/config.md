...
Bulọọku aabo fayegba ọ lati ṣe aami ibuwọlu naa ni akanṣe. To enable [JWT (json web token)](https://jwt.io/) new signture you need to add the block `jwt` to `api` section, `web` uses by default `jwt`.
